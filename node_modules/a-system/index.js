...
exports.parseJsonOrder = function(text) {
  ...
  /**
   * Часто используемые валидаторы
   */
  var validators = {
    /**
     * Фамилии с сокращенными инициалами
     * 
     * Роженко
     * Роженко Ю
     * Роженко Ю.
     ...
     * Роженко Ю. М.
     */
    russianPerson: function(val) {
      return val.match(/^[А-Я][а-я]+( ?[А-Я]?\.?){0,2}/) ? true : false;
    }
  };
  ...
  var matchers = {
    ...
    designer: {
      ...
      isValid: validators.russianPerson
    },
    /**
     * 
     */
    order_name: {
      ...
      postProcess: function(params) {
        ...
        var value = multiLineMatch(startFrom, text, params).join('');
    ...
    /**
     * Замысел: раздел "Состав изделия" состоит из двух таблиц с 4 и 5 колонками
     * Парсим четырехколоночник, пока не встретим линию с 5 объектами. Это есть
     * конец первой таблицы. В этот момент у нас будут все детали.
     * Имея количество деталей, парсим пятиколоночник.
     */ 
    order_parts: {
      matcher: /СОСТАВ ИЗДЕЛИЯ/,
      postProcess: function(p) {
        'use strict';
        let weHere = text[p.pageNumber];
        let details = [];
        /**
         * Парсим таблицу 4
         */
        // Прицеливаемся на заголовок
        let nextLine = p.lineNumber + 1;
        if (weHere[nextLine][0].txt !== 'Деталь') {
          nextLine++;
        }
        ...
        do {
          nextLine++;
          let line = weHere[nextLine];
          if (line.length === 5) {
            break; // Конец таблице 4
          }
          if (line.length === 4) {
            detail = {};
            detail.title = line[0].txt;
            detail.size = line[2].txt;
            details.push(detail);
            continue;
          }
          if (line.length === 1) {
          ...
          // @todo Не 5, не 4, и не 1 -- лажа какая-то...
          break;
        } while (true);
        
        // Парсим паспорта, чистим title
        // @todo REFACTOR!
        _.forEach(details, (d, i, c) => {
          var passFile = d.title.match(/Файл из паспорта : ([^ ,]+)/);
          if (passFile && passFile[1]) {
            c[i].passfile = passFile[1];
            c[i].title = c[i].title.replace(passFile[0], '');
          }
          var passport = d.title.match(/Присвоить паспорту №: ([^ ,]+)/);
          if (passport && passport[1]) {
            c[i].passport = passport[1];
            c[i].title = c[i].title.replace(passport[0], '');
          }
          var lead = c[i].title.match(/\d+ [^ ]*/);
          if (lead) {
            c[i].title = c[i].title.replace(lead[0], '');
          }
          var lastComma = c[i].title.match(/, *$/);
          if (lastComma) {
            c[i].title = c[i].title.replace(lastComma[0], '');
          }
          c[i].title = c[i].title.trim();
